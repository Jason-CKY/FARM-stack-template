...
from app.schemas.Todo import Todo, UpdateTodo, ReadTodo, CreateTodo
...
@router.post(
    "/todo", response_model=ReadTodo, response_description="Add new Todo", status_code=status.HTTP_201_CREATED
)
async def post_todo(todo: CreateTodo = Body(...), user: UserRead = Depends(current_active_user)):
...
async def get_todo_by_id(id: PydanticObjectId, user: UserRead = Depends(current_active_user)):
...
async def put_todo(id: PydanticObjectId, todo: UpdateTodo = Body(...), user: UserRead = Depends(current_active_user)):
...
async def remove_todo(id: PydanticObjectId, user: UserRead = Depends(current_active_user)):
